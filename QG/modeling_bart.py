...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def forward(
        ...
        **unused,
        ...
        # encoder last hidden state + decode all hidden states
        # cur_values=torch.cat((self.vhead(outputs[-1][:,-1,:]), self.vhead(outputs[0]).squeeze(-1)),dim=1)
        cur_values=self.vhead(outputs[0]).squeeze(-1)
        ...
        if not return_dict:
            output = (lm_logits,) + outputs[1:] + (cur_values,)
