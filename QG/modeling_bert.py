...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        pooled_output = self.sequential(first_token_tensor)
...
class BertQuestionAnswerPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(3*config.hidden_size, config.hidden_size)
        # self.dens2 = nn.Linear(config.hidden_size+384*2, config.hidden_size)
    ...
    def forward(self, position_ids, hidden_states):
        ...
        # answer: 2
        # context: 3
        question_token_tensors=[]
        answer_token_tensors=[]
        for ii, hidden in enumerate(hidden_states):
            pos_id=position_ids[ii]
            question_token_tensors.append(torch.mean(hidden[pos_id==1],dim=0,keepdim=True))
            answer_token_tensors.append(torch.mean(hidden[pos_id==2],dim=0,keepdim=True))
            # context_token_tensors.append(torch.mean(hidden[pos_id==3],dim=0,keepdim=True))
        question_token_tensors=torch.cat(question_token_tensors,dim=0)
        answer_token_tensors = torch.cat(answer_token_tensors, dim=0)
        # context_token_tensors = torch.cat(context_token_tensors, dim=0)
        context_token_tensors=hidden_states[:, 0]

        pooled_output = self.dense(torch.cat((question_token_tensors,answer_token_tensors,context_token_tensors),dim=1))
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config,add_pooling_layer=False)
        # self.pooler=BertQuestionAnswerPooler(config)
    ...
    def forward(
        ...
        # pooled_output = self.pooler(qa_position_ids,outputs[0])
        # pooled_output = self.pooler(outputs[0])
        pooled_output = self.pooler(outputs[0],marginal_values)
